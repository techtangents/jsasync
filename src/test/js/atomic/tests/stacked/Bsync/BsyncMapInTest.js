...
var test = forEach2_(testInts, testFunctionsFromInt, function(input, f) {
    ...
    check(Bsync.identity.mapInAsync(Async.sync(f))(input), f(input));

    // input -> identity -> f = f(input)
    check(Bsync.sync(f).mapIn(id)(input), f(input));
    check(Bsync.sync(f).mapInAsync(Async.sync(id))(input), f(input));

    // input -> identity -> identity -> input
    check(Bsync.sync(id).mapIn(id)(input), input);
    check(Bsync.sync(id).mapInAsync(Async.sync(id))(input), input);

    // input -> f -> f -> f(f(input)
    check(Bsync.sync(f).mapIn(f)(input), f(f(input)));
    check(Bsync.sync(f).mapInAsync(Async.sync(f))(input), f(f(input)));
});
